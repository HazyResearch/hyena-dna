    ...
    if n <= max_m:
        ...
        if activation is None:
            ...
            k_f = torch.view_as_complex(
                act_fn(
                    torch.view_as_real(
                        torch.einsum("... n o, ... o m -> ... n m", mat, k_f)
                    )
                )
            )  # (.., n, m)
    else:
...
class BlockFFT(OptimModule):
    ...
    def __init__(
        self,
        learn_dft_matrices=True,
        H=1,
        max_m=16,
        dft_lr=0.001,
        dropout=0,
        learn_additive=False,
        **block_fft_args,
    ):
        super().__init__()
    ...
    def forward(self, x, N, forward=True):
        ...
        if forward:
            return block_fft(
                x, N, dft_matrix=self.compute_dft_matrix, **self.block_fft_args
            )
        else:
            return (1 / (N)) * torch.conj(
                block_fft(
                    torch.conj(x),
                    N,
                    dft_matrix=self.compute_dft_matrix,
                    **self.block_fft_args,
                )
            )
