        ...
        if self.use_ma_smoothing:
            if smooth_freq:
                ...
                self.smooth = torch.nn.AvgPool1d(
                    kernel_size=self.ma_window_len, stride=1, padding=padding
                )
        ...
        if self.weight_init == "random":
            ...
            for i in range(self.H):
                for j in range(self.L):
                    double_exp[i, j] = torch.exp(
                        -(j / self.L)
                        * torch.pow(
                            torch.tensor(int(self.H / 2)), torch.tensor(i / self.H)
                        )
                    )
            K = torch.einsum("c h l, h l -> c h l", K, double_exp)
            return K
        else:
            raise NotImplementedError(f"{self.weight_init} is not valid")
    ...
    def forward(self, **kwargs):
        ...
        if self.use_ma_smoothing:
            if self.smooth_freq:
                ...
                k_f = F.conv1d(
                    k_f,
                    self.smooth_weight.to(k_f.device),
                    padding="same",
                    groups=self.H,
                )
