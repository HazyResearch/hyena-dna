...
class HG38(SequenceDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.max_length_val = (
            max_length_val if max_length_val is not None else max_length
        )
        self.max_length_test = (
            max_length_test if max_length_test is not None else max_length
        )
...
class GenomicBenchmark(HG38):
    ...
    def setup(self, stage=None):
        ...
        self.dataset_train, self.dataset_val = [
            ...
            for split, max_len in zip(
                ["train", "val"], [self.max_length, self.max_length_val]
            )
        ]
...
class NucleotideTransformer(HG38):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.shuffle = shuffle
        self.shuffle_eval = (
            shuffle_eval if shuffle_eval is not None else shuffle
        )  # default is to use the same as train shuffle arg
        self.pin_memory = pin_memory
        self.drop_last = drop_last
    ...
    def val_dataloader(
        ...
        """The val dataloader"""
        return self._data_loader(
            self.dataset_val, batch_size=self.batch_size_eval, shuffle=self.shuffle_eval
        )
